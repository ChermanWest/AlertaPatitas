...
async function publicar() {
 ...
 // 3. Construir FormData (Django necesita multipart, no base64)
...
try {
    ...
    if (fotosConArchivo.length > 0) {
        formData.append('imagen', fotosConArchivo[0].file);
    }
    ...
    mostrarFeedback('error', '❌ ' + (err.message || 'Error al guardar.'));
} finally {
    setBtnLoading(false);
  }
}